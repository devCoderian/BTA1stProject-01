...
const ConfirmMenm = ({ params }) => {
    ...
    return (
        <Container>
        <Warning>
            비밀번호 복구 단어의 순서를 맞춰주세요.
        </Warning>
        <Empty>
        {inputWords.map((item) => <EmptyWord>{item}</EmptyWord>)}
        </Empty>
        <WordList>
            {
                randomWords?.map((item, index) => {
                    return (
                        <>
                        <Word toggle={item.toggle} key={item.word} onClick={() => {
                             
                            console.log('toggle', item.toggle);
                            if(item.toggle){
                                let filteredWord = inputWords.filter(word => word !== item.word);
